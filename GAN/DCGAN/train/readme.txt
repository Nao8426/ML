...
ジェネレータの出力層は活性化関数として"Tanh"を用いることが一般的な気もするが，（学習モデルを使用するときの正規化をどうするのか微妙にややこしかったので）ディスクリミネータに合わせて"Sigmoid"を使用してます（多分，理論上勾配消失起きやすくなるかもしれんけど，そこまで変わらんはず・・・）．"Tanh"を用いるなら，画像の正規化を0~1ではなく-0.5~0.5に変えて下さい．
